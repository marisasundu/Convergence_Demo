...
function populateCommunitySuggestions() {
    ...
    // Get suggestions and shuffle them
    const chosenSuggestions = [...popularIdeasMap[selectedValue] || []];
    for (let i = chosenSuggestions.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [chosenSuggestions[i], chosenSuggestions[j]] = [chosenSuggestions[j], chosenSuggestions[i]];
    }
    
    chosenSuggestions.forEach((idea) => {
  ...
  function generateFinalSuggestions(value) {
	...
	if (continueButtonFrame2) {
		continueButtonFrame2.addEventListener('click', () => {
			...
			// Update Frame 5's idea card
			const frame5Card = document.getElementById('ideaCardFrame5');
			if (frame5Card) {
				frame5Card.innerHTML = selectedIdeaHTML;
			}
...
frame6.classList.add('active');
    ...
    if (continueButtonFrame6) {
      continueButtonFrame6.addEventListener('click', () => {
        ...
        // Call our function to populate Frame 7 based on the value selected in Frame 5
        populateCommunitySuggestions();
        
        // Make sure Frame 7's value tag matches the selected value
        const valueTagFrame7 = document.getElementById('valueTagFrame7');
        if (valueTagFrame7) {
    ...
    if (backButtonFrame2) {
      backButtonFrame2.addEventListener('click', () => {
        ...
        // Optionally, preserve the user's input
        const userIdea = document.getElementById('userIdea');
        if (userIdea) {
          userIdea.value = userIdeaText;
        }
      });
    }

    // Fix Frame 5 back button
    if (backButtonFrame5) {
      backButtonFrame5.addEventListener('click', () => {
        frame5.classList.remove('active');
        frame2.classList.add('active');
      });
    }
